...
把旅游攻略渲染成**手机竖屏、可分享**的 PDF。本文件管**流程、工具链、机械契约、验证门**；**视觉样式不在这里约束，留给模型自由发挥**——调性、配色、版式按场景自己定，**不要照搬模板**。（`docs/style-reference.md` 只是没思路时翻一下的灵感库，**默认别读**——照着规定式参考做反而容易出模板化、死板的产物。）
...
3. **写 HTML 源**：分节、每节唯一 `id`、TOC/行程速览用 `<a href="#id">`。**样式自由发挥，别照搬模板**。
...
- **CJK 字体栈/回退**：用 `-apple-system, "PingFang SC", "STSong", "Helvetica Neue", Arial, sans-serif`，回退顺序 `SC→JP→KR→Latin`（日文在前会让简体用日文字形），字型最多两种；webfont 截图前 `await document.fonts.ready`。
- **颜色保真**：`print-color-adjust: exact` 挂 `*`（挂 body 无效）。
- **图表少用**：攻略一般不用图表；万一用 Chart.js，必须 `animation:false` 且等渲染完再截，否则会截到空白/半成品。
...
## 样式（自由发挥，不在本文件约束）
调性、配色、卡片/标签/callout/emoji、字号行距——**模型按场景自由发挥，不要照搬任何模板**。只要过上面的机械契约和验证门即可。
`docs/style-reference.md` 里有配色菜单、组件骨架、避免的"AI 默认观感"等参考，但**默认别读**：照着规定式参考做反而容易出模板化、死板的产物。仅在你完全没思路、需要找灵感时翻一下，且**翻了也别照抄**。
