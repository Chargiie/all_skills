...
## 屏卡密度分档（数值菜单 · 锚点非铁律）
> SKILL.md 定「分档原则」（内容多/少跟随调整）；这里给**数值区间**当锚点。**可覆盖、可混搭**，只要过机械契约（不溢出、填充 60–100%、单卡自洽）。**一张卡选一档，别全程一套参数。**

| 档 | 用在 | 一卡块数 | 大标题 | 正文 | 行距 | 块间距 | 边距 | 手法 |
|---|---|---|---|---|---|---|---|---|
| **稀疏** | 封面 / 章节卡 / 单点 hero | 1–3 | 40–64px 粗 | 18–22px | 1.6–1.8 | 20–32px | 28–40px | 居中焦点、大留白、一个视觉主角；可整卡 `justify-content:center` |
| **均衡** | 标准点位卡 / 2–3 块 | 3–5 | 28–36px | 16–19px | 1.5–1.7 | 12–18px | 18–24px | 卡片化、标题+卡列流、舒适呼吸 |
| **密集** | 路线时间轴 / 长清单 / 对照表 | 5–9 | 22–28px | 15–17px | 1.35–1.5 | 6–11px | 14–18px | 紧凑、虚线分隔、必要时两栏；**正文别低于 15px、行距别低于 1.35**（可读下限） |

判档：先把要放的内容堆进卡，**溢出就降一档**（或拆卡），**填不满就升一档**（放大/加留白），直到 `render_mobile.cjs` 报 `fill` 落在 ~0.6–1.0 且不溢出。
...
.screen {                                                  /* 一卡一屏一页 */
  ...
  display:flex; flex-direction:column;                     /* 纵向流，留白靠 padding/gap */
  padding:28px 20px; position:relative;
  break-after:page;                                        /* 1 卡 = 1 页 */
  background:#FAF6EC;
}
.screen:last-child { break-after:auto; }                   /* 末卡不拖空白页 */
.screen.hero { justify-content:center; text-align:center; }/* 稀疏 hero：垂直居中焦点 */
.screen .grow { flex:1; }                                  /* 撑开把页脚顶到底 */
```
